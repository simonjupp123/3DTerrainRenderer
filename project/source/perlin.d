...
float perlinNoise(float x, float y) {
    ...
    float topRight = Dot(genRandomVector(x1, y0),vec2(x-x0, y-y0));
    float bottomLeft = Dot(genRandomVector(x0, y1), vec2(x-x0, y-y0));
    float bottomRight = Dot(genRandomVector(x1, y1), vec2(x-x0, y-y0));
